        ...
        >
          {slideImages.length > 0 ? (
            slideImages.map((image) => (
              <SwiperSlide key={image.id} className="rounded-lg overflow-hidden">
                <Image
                  src={image.src}
            ...
            !loading && <div className="text-center text-gray-500 p-10">Tidak ada gambar untuk ditampilkan.</div>
          )}
        </Swiper>
        {error && !loading && slideImages.length === 0 && (
          <p className="text-center text-red-500 mt-4">Gagal memuat gambar slide: {error}</p>
        )}
      </div>
    </section>
  );
